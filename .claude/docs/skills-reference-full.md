...
| `github-readme` | Produce judge-ready, visual GitHub READMEs for public/OSS/hackathon repos. Covers: README structure (hero image, badges, feature/status table, live proof table, architecture), deterministic visual path (hero banner + architecture infographic via Chrome headless + sips — no image-gen API required), and ship flow (asset placement, `curl` reachability check, CI badge verification, multi-account push gotcha). Honesty rules baked in: never invent hashes/metrics, mark Shipped vs Roadmap explicitly, mandatory THIRD_PARTY_NOTICES attribution. Triggers: "write a README", "hackathon README", "hero image for repo", "architecture diagram", "public repo README". Global skill at `~/.claude/skills/github-readme/`. |
